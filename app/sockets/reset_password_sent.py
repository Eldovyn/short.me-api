...
def register_reset_password_sent_socketio_events(socket_io: SocketIO):
    countdowns = {}
        ...
        while True:
            ...
            if remaining <= 0:
                socket_io.emit(
                    ...
                    {"remaining": 0},
                    room=room,
                    namespace="/reset-password-sent",
                )
                if data_token := ResetPasswordModel.objects(token_web=token).first():
                    data_token.delete()
                socket_io.emit(
                    "expired",
                    {"status": "expire"},
    ...
    @socket_io.on("connect", namespace="/reset-password-sent")
    def handle_connect():
        print(f"User connected from IP: {request.remote_addr}")

    @socket_io.on("disconnect", namespace="/reset-password-sent")
    def handle_disconnect():
        print(f"User disconnected from IP: {request.remote_addr}")

    @socket_io.on("join", namespace="/reset-password-sent")
    def handle_join(data):
        ...
        user_token = ResetPasswordModel.objects(token_web=token).first()
        if not user_token:
            disconnect()
            return

        room = f"reset-password-sent-{user_token.id}"
        join_room(room)
