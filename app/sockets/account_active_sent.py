...
def register_account_active_sent_socketio_events(socket_io: SocketIO):
    countdowns = {}
    ...
    @socket_io.on("connect", namespace="/account-active-sent")
    def handle_connect():
        print(f"User connected from IP: {request.remote_addr}")

    @socket_io.on("disconnect", namespace="/account-active-sent")
    def handle_disconnect():
        print(f"User disconnected from IP: {request.remote_addr}")

    @socket_io.on("join", namespace="/account-active-sent")
    def handle_join(data):
        ...
        user_token = AccountActiveModel.objects(token_web=token).first()
        if not user_token:
            disconnect()
            return

        room = f"account-active-sent-{user_token.id}"
        join_room(room)
